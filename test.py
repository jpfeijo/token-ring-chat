...
def receive():  # função em loop para receber as mensagens
    while True:
        ...
        # quando recebe, chama a função para tratar a mensagem
        handleMessage(message)


def handleMessage(messageRaw):
    global tokenReceivedTime, userStartedWithToken, messageSent, userHasToken, dataMessages, removeToken
...
# precisa disso?
        ...
        # se tiver mensagem para enviar, ele envia a mensagem e espera uma resposta
        messageSent = True
        sendMessages()
    ...
    if message.startswith("7777"):  # se receber um pacote de dados
        ...
        if destination == userName:  # se o destino for o usuario
            ...
            if calculatedCrc != crc:
                ...
                print(f'Mensagem recebida de {source}: ', messageContent)
                print()
                passAlongMessages(forwardMessage(
            ...
            if destination == 'TODOS':  # se a mensagem for broadcast, ele printa e repassa a mensagem
                print(f'(Broadcast de {source}):', messageContent)
...
# função para repassar a mensagem para o proximo do anel
...
def sendMessages():  # função para enviar as mensagens
    ...
    # se a mensagem for privada, ele atualiza o destino e a mensagem
    if message.startswith('@'):
        ...
        # se a mensagem for broadcast, ele atualiza a mensagem
...
# função para empacotar a mensagem recebendo crc como parametro
...
# thread para escrever as mensagens
threading.Thread(target=writeMessages).start()
# thread para cuidar do tempo de timeout
